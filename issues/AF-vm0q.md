...
content_hash: "sha256:e9ae318f5c23d36e22664ee32e6b7071a6f0e0fa9f4051c4c19625434814a424"
blocked_by: [AF-8r8l, AF-6jta, AF-pfbv, AF-o0rw, AF-c17x, AF-4wkn]
was_blocked_by: [AF-q5yh, AF-iv8x, AF-hb2f]
---

## Description
Description:
Static verification capstone for the whole `arr-stack` manifest set: prove the committed manifests describe a coherent, deployable system end to end -- including every cross-file contract that would otherwise only break at runtime -- without requiring a live cluster on the critical path. This is the epic's capstone: it introduces no new deploy-facing code, it proves everything the other developer-claimable and human-gated stories (AF-hb2f, AF-8r8l, AF-iv8x, AF-6jta, AF-pfbv, AF-o0rw, AF-c17x, AF-4wkn) built actually fits together as one system end to end, catching anything that passed its own story's narrower verification but breaks when everything -- including the live evidence -- is combined.
...
USER INTENT:
Anyone reviewing this epic's delivery needs one command that either says "the whole arr-stack manifest set is internally coherent" or points at the exact cross-file contract that's broken -- not five separate spot-checks that each pass in isolation while the system as a whole is subtly wrong (a 7th app added to one list and not the other, a `hasDownloads` mismatch between the parameter table and the rendered template, a stray reference to an out-of-scope app, or a digest silently bound into the wrong app-template field). Running the suite displays a clear pass/fail per section; the user can trust a clean run as the epic's actual coherence proof, not an agent's paraphrase of one.
...
- Correcting the design spec doc's own snippet (`docs/superpowers/specs/2026-08-18-arr-stack-appset-design.md` lines ~203-205) -- tracked as a documentation-only follow-up outside the execution path.

DIFF BUDGET:
1 file modified (`e2e/observability_test.rb`), 0 new files. Expect roughly 150-260 added LOC (a new section comparable in size to the existing 150-assertion file's largest section; slightly larger than the original estimate to cover the digest/tag regression checks added by this bug-triage pass).
...
- AF-6jta: apps/arr-stack/argocd/appset-workloads.yaml -> matrix-generator ApplicationSet
    spec: container image block binds digest: "{{.values.imageTag}}" (not tag:)
...
3. [Ubiquitous] All cross-file contract checks (item 3) pass, including the `imageTag` digest-shape check and the `digest:` (not `tag:`) binding check.
4. [Unwanted] All negative assertions (item 4) pass, including the new digest-regression check.
5. [Ubiquitous] The repo-wide collision check (item 5) and quality-gate check (item 6) pass.
6. [Ubiquitous] Self-validation (item 7) is performed with at least 6 deliberate regressions, one of which is the `digest:`-to-`tag:` reversion, and each is caught by its specific assertion; this is recorded as delivery evidence, not asserted without detail.
...
## Links
- Parent: [[AF-j5rz]]
- Blocked by: [[AF-8r8l]], [[AF-6jta]], [[AF-pfbv]], [[AF-o0rw]], [[AF-c17x]], [[AF-4wkn]]
- Was blocked by: [[AF-q5yh]], [[AF-iv8x]], [[AF-hb2f]]
