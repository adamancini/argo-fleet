...
content_hash: "sha256:20d74a617d013d4fd7c7ba6df99a73d8883c05577008573d53477624162c4aa7"
blocked_by: [AF-8r8l, AF-iv8x, AF-6jta, AF-pfbv, AF-o0rw, AF-c17x, AF-4wkn, AF-hb2f]
...
USER INTENT:
Anyone reviewing this epic's delivery needs one command that either says "the whole arr-stack manifest set is internally coherent" or points at the exact cross-file contract that's broken -- not five separate spot-checks that each pass in isolation while the system as a whole is subtly wrong (a 7th app added to one list and not the other, a `hasDownloads` mismatch between the parameter table and the rendered template, a stray reference to an out-of-scope app). Running the suite displays a clear pass/fail per section; the user can trust a clean run as the epic's actual coherence proof, not an agent's paraphrase of one.
...
5. [Ubiquitous] Every file under `kargo-chart/` carries the `+argocd:skip-file-rendering` marker (independently re-verified, not trusted from AF-hb2f's own claim).
...
## Links
- Parent: [[AF-j5rz]]
- Blocked by: [[AF-8r8l]], [[AF-iv8x]], [[AF-6jta]], [[AF-pfbv]], [[AF-o0rw]], [[AF-c17x]], [[AF-4wkn]], [[AF-hb2f]]
